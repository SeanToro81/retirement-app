...
def run_retirement(prefix, include_spouse=False):
    ...
    with col1:
        ...
        monthly_retirement_income = st.number_input("Desired Monthly Income in Retirement (today's $)", 0, 100_000, 4_000, 500,
    ...
    current_withdrawal = monthly_retirement_income
    depleted_month = None
    ...
    for m in range(1, draw_months + 1):
        ...
        net_withdrawal = max(0, current_withdrawal - total_other_income)
    ...
    if not include_spouse:
        ...
        with s1:
            st.markdown("**Before 65**")
            st.caption("No CPP or OAS yet")
            st.metric("CPP + OAS", "$0/mo")
            st.metric("Drawn from Portfolio", f"${monthly_retirement_income:,.0f}/mo")
